...
public class MainActivity extends Activity {
    ...
    private void showSplashThenLoad() {
        try {
            ...
            splash.setImageBitmap(BitmapFactory.decodeStream(getAssets().open("alx-splash.png")));
            splash.setBackgroundColor(android.graphics.Color.BLACK);
            setContentView(splash, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    private class AndroidFiles {
        @JavascriptInterface public void open3DStudio() {
            runOnUiThread(() -> {
                Intent intent = new Intent(MainActivity.this, com.alx.designstudio3d.MainActivity.class);
                startActivity(intent);
            });
        }

        @JavascriptInterface public void sendCreativeAssetTo3D(String dataUrl, String name, String kind) {
            new Thread(() -> {
                try {
                    int comma = dataUrl.indexOf(',');
                    byte[] bytes = Base64.decode(dataUrl.substring(comma + 1), Base64.DEFAULT);
                    File shared = new File(getFilesDir(), "alx_shared"); if (!shared.exists()) shared.mkdirs();
                    String safe = name.replaceAll("[^a-zA-Z0-9._-]", "_");
                    File image = new File(shared, safe.endsWith(".png") ? safe : safe + ".png");
                    try (FileOutputStream fos = new FileOutputStream(image)) { fos.write(bytes); }
                    File meta = new File(shared, "latest_asset.json");
                    String json = "{\"name\":\""+safe+"\",\"kind\":\""+kind+"\",\"path\":\""+image.getAbsolutePath().replace("\\","/")+"\"}";
                    try (FileWriter fw = new FileWriter(meta)) { fw.write(json); }
                    runOnUiThread(() -> {
                        Toast.makeText(MainActivity.this, "Recurso enviado a Diseño 3D", Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(MainActivity.this, com.alx.designstudio3d.MainActivity.class);
                        intent.putExtra("sharedAssetPath", image.getAbsolutePath());
                        intent.putExtra("sharedAssetKind", kind);
                        startActivity(intent);
                    });
                } catch (Exception e) { runOnUiThread(() -> Toast.makeText(MainActivity.this, "No se pudo transferir el recurso", Toast.LENGTH_LONG).show()); }
            }).start();
        }
        ...
        @JavascriptInterface public void saveUnifiedProject(String json) {
            new Thread(() -> {
                try {
                    File f = new File(getFilesDir(), "current.alxcds");
                    try (FileWriter fw = new FileWriter(f)) { fw.write(json); }
                    runOnUiThread(() -> Toast.makeText(MainActivity.this, "Proyecto 2D/3D unificado guardado", Toast.LENGTH_SHORT).show());
                } catch (Exception e) { runOnUiThread(() -> Toast.makeText(MainActivity.this, "No se pudo guardar el proyecto unificado", Toast.LENGTH_LONG).show()); }
            }).start();
        }
    ...
    @Override protected void onNewIntent(Intent intent) {
        ...
        String path = intent.getStringExtra("render3DPath");
        if (path != null) {
            new Thread(() -> {
                try {
                    byte[] bytes = Files.readAllBytes(new File(path).toPath());
                    String data = "data:image/png;base64," + Base64.encodeToString(bytes, Base64.NO_WRAP);
                    String name = new File(path).getName().replace("'", "");
                    runOnUiThread(() -> webView.evaluateJavascript("window.import3DRenderAsLayer('"+data+"','"+name+"')", null));
                } catch(Exception e) { runOnUiThread(() -> Toast.makeText(this,"No se pudo abrir el render 3D",Toast.LENGTH_LONG).show()); }
            }).start();
        }
    }
